...
void adaptive_astar(grid_cell **grid_cell_object,
	...
	int i = 0;
	copy_previous_g(grid_cell_object,
							closed_list);
	//print the h,g values of gridcell after first astar call.
	// std::cout<<"h & g before h value update"<<"\n";
	// 		printf("%s\n", "1st A star");
	// printHG(grid_cell_object,grid_height,grid_width,path_vector);

	update_h_val(grid_cell_object,
							closed_list,path_vector);
	// std::cout<<"h & g after h value update"<<"\n";
	// 		printf("%s\n", "1st A star");
	// printHG(grid_cell_object,grid_height,grid_width,path_vector);
